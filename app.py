...
def player_matches(df, player, surface="All"):
    ...
    w["result"] = pd.Series("", index=w.index, dtype="object")
    w.loc[w["winner_code"].eq(1), "result"] = "W"
    w.loc[w["winner_code"].eq(2), "result"] = "L"
    ...
    l["result"] = pd.Series("", index=l.index, dtype="object")
    l.loc[l["winner_code"].eq(2), "result"] = "W"
    l.loc[l["winner_code"].eq(1), "result"] = "L"
...
if pd.notna(max_date):
    st.caption(f"Loaded match data through {max_date.strftime('%d %b %Y')}. This local dataset is not live; replace the CSVs or connect an API for newer results.")
