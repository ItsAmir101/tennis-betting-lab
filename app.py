...
def github_raw(repo, path, timeout=45):
...
def load_csv(repo, path):
    data = github_raw(repo, path)
...
def load_players(tour):
    repo = REPOS[tour]
    path = 'atp_players.csv' if tour == 'ATP' else 'wta_players.csv'
    p = load_csv(repo, path)
    # Jeff Sackmann's player files use name_first/name_last.
    first_col = 'name_first' if 'name_first' in p.columns else 'first_name'
    last_col = 'name_last' if 'name_last' in p.columns else 'last_name'
    if first_col in p.columns and last_col in p.columns:
        p['display'] = p[first_col].fillna('').astype(str).str.strip() + ' ' + p[last_col].fillna('').astype(str).str.strip()
    elif 'name' in p.columns:
        p['display'] = p['name'].astype(str)
    else:
        raise RuntimeError(f'Could not identify player-name columns in {path}.')
    p['display'] = p['display'].str.replace(r'\s+', ' ', regex=True).str.strip()
    return p[p['display'].ne('')].drop_duplicates('display').sort_values('display')
...
def load_all_matches(tour):
    ...
    for year in YEARS:
        try:
            frames.append(load_year(tour, year))
        except Exception as e:
            errors.append(str(e))
    if not frames:
        raise RuntimeError('No match files could be downloaded through the public GitHub API. Check Streamlit network access.')
    return pd.concat(frames, ignore_index=True, sort=False)
...
with st.sidebar:
    ...
    st.write('Source: Jeff Sackmann ATP/WTA public repositories')
    st.write('Transport: GitHub public Contents API')
    st.caption('The app reads the official public repository files through GitHub instead of raw.githubusercontent.com. This avoids the network failure affecting your previous deployment.')
...
try:
    ...
    st.info('This version uses the GitHub public Contents API. If this error appears, open Manage app → Logs and send me the exact error text.')
    st.stop()
