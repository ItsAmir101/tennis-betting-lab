...
def parse_kalshi_url(url):
    ...
    # Kalshi links generally end with the real market ticker after the
    # human-readable match slug.
...
def kalshi_market_from_url(url):
    ...
    # Kalshi sports markets can live on a different exchange shard.
    # Direct GET /markets/{ticker} can therefore return 404 even when the
    # market is live. The documented /markets endpoint supports a ticker
    # filter and is the safer public lookup for this app.
    ...
    # Fallback: try the direct endpoint only if the filtered list did not
    # return it (useful for older/edge-case markets).
    data2, err2 = kalshi_get(f"/markets/{ticker}")
    if not err2 and data2.get("market"):
        return data2["market"], None

    return None, (
        f"Kalshi found no market for ticker {ticker}. "
        "This can happen if the market is closed/archived or the ticker is invalid."
    )
